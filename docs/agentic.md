...
The `templates/architecture-tests/autonomy-guard.test.js` template can be run as a pre-commit or CI gate, catching any changes that touch `humans-only` paths regardless of which agent made them:
...
Even if a sub-agent skips `verify-ci.sh`, the GitHub Actions workflow in `templates/github-actions/ci.yml` catches it at PR time. The pipeline runs the same checks — no agent can merge code that fails them.
...
**`clear_verify`** — runs `scripts/verify-ci.sh` and returns structured results:
```json
{
  "status": "failed",
...
Use the skill template at `templates/skills/mcp-server.md` to generate a minimal MCP server for your project. The template generates a Node.js or Python server that wraps these three tools.
...
2. Run `./scripts/verify-ci.sh` (or call `clear_verify` via MCP) as the final step
3. Fail loudly (non-zero exit) if verification fails — let the pipeline catch it

The GitHub Actions template at `templates/github-actions/ci.yml` is exactly this pattern: a headless agent that runs `verify-ci.sh` on every PR.
...
- [ ] `scripts/verify-ci.sh` tested and passing on current codebase
...
- [ ] (Optional) CLEAR MCP server scaffolded from `templates/skills/mcp-server.md`
...
| MCP server skill template | [templates/skills/mcp-server.md](../templates/skills/mcp-server.md) |
