...
#
# Usage:
#   ./clear/verify-ci.sh           # Run all checks
#   ./clear/verify-ci.sh --fast    # Skip slow checks (architecture tests)
#   ./clear/verify-ci.sh --fix     # Auto-fix linting issues where possible
#
# Customization:
#   Add project-specific checks to clear/verify-local.sh (not this file).
...
check_autonomy() {
  ...
  if [[ -f "$PROJECT_ROOT/clear/autonomy.yml" ]]; then
    ...
    warn "clear/autonomy.yml not found — run the CLEAR installer to configure CLEAR"
  fi
}
